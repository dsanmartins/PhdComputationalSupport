...
public aspect UtilsKDM {
	...
	after(Resource resource, String javaProjectName) : utilsKDM(resource, javaProjectName)  {
		...
		generateStructure.annotationClass(new File(path));
		generateStructure.annotationFieldClass(new File(path));
		generateStructure.annotationMethod(new File(path));
		generateStructure.annotationVariable(new File(path));
		try {generateStructure.addAggregatedRelationShip(new File(path));} catch (Exception e) {e.printStackTrace();} 
